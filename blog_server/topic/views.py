    ...
    if request.method == "POST":
        ...
        if t_id:
            ...
            if visitor_username == author_id:
                ...
                #博主访问自己的博客
                try:
                    author_topic = Topic.objects.get(id=t_id) # author_topic 是可以访问到的所有博客,包含公开的和私有的
                except Exception as e:
            ...
            res = make_topics_res(author, author_topics)#将生成结果专门定义一个函数,make_topics_res()
            return JsonResponse(res)
...
#-------------------------------------------------------------------
    ...
    all_messages = Message.objects.filter(topic=author_topic).order_by('-created_time') #author_topic 是可以访问到的所有博客,包含公开的和私有的
    # leve1_msg = {1:[m1,m2,m3], 3,:[m4,m5,m6]}
    # messages = [ma_1, mb_3, mc_5 ]

    msg_list=[]#即messages,所有的留言和回复 message[{.....}]

    # key：是留言id, value是[回复对象,回复对象]
    level1_msg = {}  # 'aaa': [] 即replay[{...}]
    # 全部留言及回复的数量
    m_count = 0
    for msg in all_messages:
        ...
        if msg.parent_message:
            # 回复
            level1_msg.setdefault(msg.parent_message, [])#msg.parent_message
            level1_msg[msg.parent_message].append({'msg_id': msg.id, 'publisher': msg.publisher.nickname,
    ...
    for m in msg_list:
        if m['id'] in level1_msg:
            # 所有留言即msg_list,其中的一个元素的id在所有的回复即level1_msg({1:[m1,m2,m3], 3,:[m4,m5,m6]})中的时候,证明此时候是有回复,所以将回复内容添加到reply里面

            m['reply'] = level1_msg[m['id']]#将reply里面添加根据m['id']（即后面字典里面的键）{1:[m1,m2,m3], 3,:[m4,m5,m6].......}得到的内容[m1,m2,m3]、[m4,m5,m6]等
#------------------------------------------------------------------------
    ...
    #暂时为假数据
    result['data']['messages'] = msg_list  #所有的留言和回复,包含有回复的和无回复的两种
